<?
    class Country_reply extends CI_Controller {
		...
		$config["total_rows"] = $this->country_reply_m->rowcount($text1);  // 전체 레코드개수 구하기
		...
		$data["text1"]=$text1;
        $data["list"] = $this->country_reply_m->getlist($text1,$start,$limit); // 해당페이지 자료읽기
		...
		if ( $this->form_validation->run()==TRUE )
		{
		 ...
		 $this->load->view("country_view");    // 입력화면 표시
